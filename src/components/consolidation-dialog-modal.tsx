...
import { ConsolidationListbox } from "../consolidations-listbox";
...
export default function UnitAssignmentDialogModal({
  isOpen,
  setIsOpen,
}: Props) {
  ...
  const [selectedConsolidation, setSelectedConsolidation] = useState(
    () =>
      initData.consolidations[0
      // initData.consolidations.findIndex((consolidation) => {
  ...
  function saveSettingsDialogModal(): void {
    ...
    //                 setUnitStatsData({
    //                   ...unitStatsData,
    //                   ...prepareStatsData(response.unitData),
    //                 });
      ...
      <div className="touch-none fixed inset-0 overflow-y-auto">
        <div className="flex min-h-full items-center justify-center p-4 text-center">
          <DialogPanel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
            ...
            >
              Przypisz się do jednostki...
            </DialogTitle>
            <ConsolidationListbox
              consolidations={initData.consolidations}
              selectedConsolidation={selectedConsolidation}
              setSelectedConsolidation={setSelectedConsolidation}
            ...
            <div className="mt-2">
              <p className="text-sm text-gray-500">
                {!fetchingData &&
                  ...
                  "Wybierz grupę, do której będziesz przypisana(y)..."}
                {!fetchingData &&
                  requestDataFailed === false &&
                  "Zostałeś przypisany do wybranej grupy."}
                {!fetchingData &&
                  requestDataFailed &&
